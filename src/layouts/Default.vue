<template>
  <div class="layout">
    <section class="page-header" :style="'background-image: linear-gradient(120deg, '+configuration.backgroundColorLeft+', '+configuration.backgroundColorRight+');color: '+configuration.fontColor+';'">
      ...
      <a :href="'https://github.com/'+configuration.githubUsername" class="btn" target="_blank">GitHub主页</a>
        ...
        <el-row>
          <el-col :span="10">
            <el-menu @select="selectTopbar" :default-active="topbar.active" mode="horizontal" menu-trigger="click">
              ...
              <!--<el-submenu index="#webSites" v-if="configuration.webSites.length>0">-->
                <!--<template slot="title">其他网站</template>-->
                <!--<el-menu-item :index="'#webSites-'+index" v-for="(item,index) in configuration.webSites" :key="'#webSites'+index">{{item.name}}</el-menu-item>-->
              <!--</el-submenu>-->
            </el-menu>
          </el-col>
          ...
          <el-col :span="4" style="text-align: right;">
            ...
            <div style="color:#606266;">
              <i class="el-icon-location"></i>&nbsp;{{configuration.location?configuration.location:'未填写地址'}}
              <br>
            </div>
          </el-col>
          <el-col :span="2" style="text-align: center;">
            <img v-popover:bigAvatar :src="configuration.avatar_url" style="margin-top: 4px;margin-right: 10px;width:52px; height:52px; border-radius:5px; border: 1px solid #EBEEF5"
            />
            <el-popover ref="bigAvatar" placement="top-start" :title="configuration.githubUsername" width="200" trigger="hover">
              <i class="el-icon-star-on"></i>&emsp;{{configuration.name}}
              <br>
              <i class="el-icon-location"></i>&emsp;{{configuration.location}}
              <br>
              <img :src="configuration.avatar_url" style="width: 200px;height: 200px;">
            </el-popover>
          </el-col>
        </el-row>
...
query {
  metadata {
    configuration{
      name,githubUsername,blogTitle,blogDescribe,htmlTitle,avatar_url,location,
      fontColor,useBackgroundImage,backgroundColorLeft,backgroundColorRight,audioUrl,mini
    }
  }
}
</static-query>
<script>
  ...
  export default {
    ...
    computed:{
			configuration(){
				return this.$static.metadata.configuration
      }
    },
		methods: {
			selectTopbar(index) {
